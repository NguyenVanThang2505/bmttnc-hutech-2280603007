...
class ECCCipher:
    ...
    def generate_keys(self):
        sk = ecdsa.SigningKey.generate()  # Sửa lỗi: Thiếu đường cong
        vk = sk.verifying_key  # Sửa lỗi: `.get_Verifying_key()` -> `.verifying_key`
    ...
    def load_keys(self):
        ...
        with open('cipher/ecc/keys/publicKey.pem', 'rb') as p:
            vk = ecdsa.VerifyingKey.from_pem(p.read())  # Sửa lỗi: `Verifying` -> `VerifyingKey`

        return sk, vk
    
    def sign(self, message, key):  # Sửa lỗi: `delf` -> `self`
